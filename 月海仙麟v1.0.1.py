            ...
            for class_ in self.class_list:
                ...
                for time in  class_.time().time_char_4+class_.time().time_wap_4:
                    if time not in class_.time().time_count_all_4.keys():
                        ...
                        class_.time().time_count_all_4[time]+=1
                class_.time().time_countlist_all_4+=sorted(Tool().dict_to_list(class_.time().time_count_all_4),key = lambda x:x[0])
                ...
                for time in class_.time().time_char_5+class_.time().time_wap_5:
                    if time not in class_.time().time_count_all_5.keys():
                        ...
                        class_.time().time_count_all_5[time]+=1 
                class_.time().time_countlist_all_5+=sorted(Tool().dict_to_list(class_.time().time_count_all_5),key = lambda x:x[0])
                #处理三星出货时间
                for time in  class_.time().time_wap_3:
                    if time not in class_.time().time_count_wap_3.keys():
                        ...
                        class_.time().time_count_wap_3[time]+=1
                class_.time().time_countlist_wap_3+=sorted(Tool().dict_to_list(class_.time().time_count_wap_3),key = lambda x:x[0])
                #处理四星出货日期         
                for date in  class_.date().date_char_4+class_.date().date_wap_4:
                    if date not in class_.date().date_count_all_4.keys():
                        ...
                        class_.date().date_count_all_4[date]+=1
                class_.date().date_countlist_all_4+=sorted(Tool().dict_to_list(class_.date().date_count_all_4),key = lambda x:x[0])
                #处理五星出货日期
                ...
                for date in class_.date().date_char_5+class_.date().date_wap_5:
                    ...
                    if date not in class_.date().date_count_all_5.keys():
                        ...
                        class_.date().date_count_all_5[date]+=1
                class_.date().date_countlist_all_5+=sorted(Tool().dict_to_list(class_.date().date_count_all_5),key = lambda x:x[0])
                #处理三星出货日期
                for date in  class_.date().date_wap_3:
                    if date not in class_.date().date_count_wap_3.keys():
                        ...
                        class_.date().date_count_wap_3[date]+=1
                class_.date().date_countlist_wap_3+=sorted(Tool().dict_to_list(class_.date().date_count_wap_3),key = lambda x:x[0])
...
class charts():#图表类
    def make_pie(self,data_class ):#构造饼图对象，传入数据类
        ...
        total_count=0
        data_=[['三星',len(data_class.wap_3)],['四星武器',len(data_class.wap_4)],['四星角色',len(data_class.char_4)],['五星武器',len(data_class.wap_5)],['五星角色',len(data_class.char_5)]]
        for i in data_:
            total_count+=i[1]
        ...
        pie = (
        pyecharts.charts.Pie(init_opts=pyecharts.options.InitOpts( chart_id=charts_name,theme=pyecharts.globals.ThemeType.MACARONS))#设置id和主题
        .add(
            charts_name,
            data_,
            ...
            tooltip_opts=pyecharts.options.TooltipOpts
                ...
                formatter=pyecharts.globals.JsCode('''function(parg){\n
                    if( parg.name == '三星')\n
                    {\n
                    return ' 三星： '''+str(len(data_class.wap_3))+' ( '+str(int((len(data_class.wap_3)/total_count)*10000)/100)+''' % )</br>'+\''''+wap3+'''\' ;\n
                                

                    }else if (parg.name == '四星武器')\n
                    {\n
                    return ' 四星武器： '''+str(len(data_class.wap_4)+len(data_class.char_4))+' ( '+str(int(((len(data_class.wap_4)+len(data_class.char_4))/total_count)*10000)/100)+''' % )</br>'+\''''+wap4+'''\' ;\n
                                
                    }else if (parg.name == '五星武器')\n
                    {\n
                    return ' 五星： '''+str(len(data_class.wap_5)+len(data_class.char_5))+' ( '+str(int(((len(data_class.wap_5)+len(data_class.char_5))/total_count)*10000)/100)+''' % )</br>'+\''''+wap5+'''\' ;\n
                                
                    }else if (parg.name == '四星角色')\n
                    {\n
                    return ' 四星角色： '''+str(len(data_class.wap_4)+len(data_class.char_4))+' ( '+str(int(((len(data_class.wap_4)+len(data_class.char_4))/total_count)*10000)/100)+''' % )</br>'+\''''+char4+'''\' ;\n
                                
                    }else if (parg.name == '五星角色')\n
                    {\n
                    return ' 五星： '''+str(len(data_class.wap_5)+len(data_class.char_5))+' ( '+str(int(((len(data_class.wap_5)+len(data_class.char_5))/total_count)*10000)/100)+''' % )</br>'+\''''+char5+'''\' ;\n
